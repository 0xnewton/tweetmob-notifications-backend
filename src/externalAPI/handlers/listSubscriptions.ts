...
export const listSubscriptions = onRequest(
  async (request: APIRequest, response: APIResponse): Promise<void> => {
    ...
    logger.info("List subscriptions event handler", { user, limit, response });
    ...
    try {
      ...
      const responsePayload: ListSubscriptionsAPIResponse = {
        data: subscriptionList.data.map((subscription) => subscription.data),
